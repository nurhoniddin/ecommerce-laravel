...
class UsersController extends Controller
{
    ...
    public function register(Request $request){
        if ($request->isMethod('post')) {
            ...
            if ($usersCount>0) {
                ...
                if (empty($data['name'])) {
                return redirect()->back()->with('flash_message_error','Please required name!');
                }

                if (empty($data['email'])) {
                return redirect()->back()->with('flash_message_error','Please required email!');
                }

                if (empty($data['password'])) {
                return redirect()->back()->with('flash_message_error','Please required password!');
                }
    ...
    public function verify(Request $request){
        ...
        return view('users.verify')->with(compact('data'));
        }
    }
    ...
    public function checkMobile(Request $request){
        	$data = $request->all();
        	// Check if User already exists
        	$usersCount = User::where('mobile',$data['mobile'])->count();
        	if ($usersCount>0) {
        		echo "false";
        	}else{
        		echo "true"; die;
