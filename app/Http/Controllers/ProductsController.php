...
class ProductsController extends Controller
{
        ...
        if($request->isMethod('post')){
            ...
            foreach($data['sku'] as $key => $val){
                if (!empty($val)) {
                    ...
                    if ($attrCountSizes>0) {
                        return redirect('admin/add-attributes/'.$id)->with('flash_message_error','"'.$data['size'][$key].'" Size already exist for this product! Please add another Size.');
                    }
    ...
    public function products($url = null){
        ...
        if ($categoryDetails->parent_id==0) {
            ...
            // print_r($cat_ids); die; 
            $productsAll = Product::whereIn('category_id',$cat_ids)->where('status',1)->paginate(2);
            //$productsAll = json_decode(json_encode($productsAll)); 
            // echo "<pre>"; print_r($productsAll); die; 
        }else{
            // If url is sub category url
            $productsAll = Product::where(['category_id' => $categoryDetails->id])->where('status',1)->paginate(2);
        }        
    ...
    public function product($id = null){
       ...
       // $relatedProducts = json_decode(json_encode($relatedProducts));
        ...
        return view('products.detail')->with(compact('productDetails','categories','productAltImages','relatedProducts','total_stock'));
    }
